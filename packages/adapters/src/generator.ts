...
function sanitizeName(name: string): string {
  ...
  return sanitized || 'adapter';
}

/**
 * Escape a string for use in a template literal.
 * Prevents injection by escaping backticks and ${.
 */
...
export function generateAdapterCode(
  ...
  const safeBaseUrl = escapeTemplateString(options.baseUrl || 'https://api.example.com');
  const auth = options.auth || 'none';

  // Group endpoints by tag or path prefix
  const methodsCode = endpoints.map(ep => generateMethodCode(ep)).join('\n\n');

  const authCode = generateAuthCode(auth, safeName);
  const fetchCode = generateFetchCode(auth);

  return `/**
 * Auto-generated adapter: ${safeName}
...
function getBaseUrl(): string {
  return process.env.${safeName.toUpperCase()}_API_URL || '${safeBaseUrl}';
}

${fetchCode}

export const ${safeName} = defineAdapter({
  name: '${safeName}',
  description: 'Auto-generated adapter for ${safeName} API',
  tools: {
${methodsCode}
  },
});
...
function generateMethodCode(endpoint: ParsedEndpoint): string {
  ...
  const paramsSchema = parameters
    ...
    .map(p => {
      ...
      const safeDesc = escapeTemplateString(p.description || '');
      return `        ${safeName}: { type: '${type}', description: '${safeDesc}', required: ${p.required || false} }`;
    })
    .join(',\n');

  // Build path with parameter substitution (sanitize parameter names)
  const pathWithParams = path.replace(/\{(\w+)\}/g, (_, name) => `\${params.${sanitizeName(name)}}`);

  // Build execute function (sanitize parameter names)
  const queryParams = parameters.filter(p => p.in === 'query').map(p => sanitizeName(p.name));

  let queryObj = '';
  if (queryParams.length > 0) {
    queryObj = `{ ${queryParams.join(', ')} }`;
  }
